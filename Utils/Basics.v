...
Definition filter
           (X : Type)
           (p : X -> Prop)
           (p_dec : forall x, dec (p x)) : list X -> list X :=
...
Hint Resolve
     rem_not_in rem_incl rem_mono
     rem_cons rem_cons' rem_app rem_app'
     rem_in rem_neq rem_inclr : core.
...
Section Dupfree.
  ...
  Proof.
    ...
    - left. left.
    - decide (x el A) as [E|E].
      + right. intros F. inverts F; tauto.
      + destruct (IHA) as [F|F].
        * unfold dec. auto using dupfree.
        * right. intros G. inverts G; tauto.
  Qed.
